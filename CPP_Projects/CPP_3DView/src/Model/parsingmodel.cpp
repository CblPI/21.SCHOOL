#include "../Headers/parsingmodel.h"
...
void ParsingModel::AfinMove(double moving, int axis) {
  for (size_t i = 0; i < object_.vert.size(); i++) {
    object_.vert[i][axis] += moving;
  }
}

void ParsingModel::AfinRotate(double angle, int axis) {
...
void ParsingModel::AfinScale(double scale) {
  for (size_t i = 0; i < object_.vert.size(); i++) {
...
void ParsingModel::VertexFill(std::string line) {
...
void ParsingModel::VertexNFill(std::string line) {
  std::vector<double> tmp_vector;
  tmp_vector = VectorEntry(tmp_vector, line);
  object_.vert_n.push_back(tmp_vector);
}

void ParsingModel::VertexTFill(std::string line) {
  std::vector<double> tmp_vector;
  tmp_vector = VectorEntry(tmp_vector, line);
  object_.vert_t.push_back(tmp_vector);
}

void ParsingModel::PolygonFill(std::string line) {
