...
TEST(test, ln) {
  s21::Model result{"ln(10)"};
  EXPECT_EQ(result.GetData(), log(10));
}

TEST(test, log) {
  s21::Model result{"log(10)"};
  EXPECT_EQ(result.GetData(), log10(10));
}

TEST(test, checking) {
  s21::Model result;
  EXPECT_EQ(result.Checking("log(10)"), true);
}

TEST(test, checking_fls) {
  s21::Model result;
  EXPECT_EQ(result.Checking("log(10+)cos(20)"), false);
}

TEST(test, checking_fls2) {
  s21::Model result;
  EXPECT_EQ(result.Checking("cos(22))"), false);
}

TEST(test, checking_fls3) {
  s21::Model result;
  EXPECT_EQ(result.Checking("cos(cos(20)))"), false);
}

TEST(test, checking_fls4) {
  s21::Model result;
  EXPECT_EQ(result.Checking("cos(cos(20)))"), false);
}

TEST(test, checking_true1) {
  s21::Model result;
  EXPECT_EQ(result.Checking("cos(sin(1))"), true);
}

TEST(test, checkng_true2) {
  s21::Model result;
  EXPECT_EQ(result.Checking("22e+10-10e+5"), true);
}

TEST(test, checkng_true3) {
  s21::Model result;
  EXPECT_EQ(result.Checking("22+5."), false);
}

TEST(test, checkng_true4) {
  s21::Model result;
  EXPECT_EQ(result.Checking("22e43"), true);
}
